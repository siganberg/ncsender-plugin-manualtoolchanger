...
PROMPT="Generate release notes for version $NEW_VERSION of the Manual Tool Change plugin from ONLY the following commit messages. Do NOT invent, assume, or add any changes not explicitly listed below.
...
9. CRITICAL — INTRA-BRANCH CHURN: The commits below are the WORK done since the last release, not the delta from the user's perspective. It is common for one commit to add a feature and a later commit in the same range to fix a bug in THAT feature. The bug never shipped to users, so it MUST NOT appear as a \"Bug Fix\" entry. Report ONLY the NET user-facing outcome — describe the finished feature, do not list the mid-development fixes to it. A \"Bug Fix\" entry belongs in the release notes ONLY when it fixes a defect that existed in the PREVIOUS release (i.e. a real regression the user could have hit)

Output ONLY the markdown. No preamble. No explanation. Just the markdown."

# Use Claude CLI to generate release notes. Capture only stdout — \`2>&1\`
# would fold stderr warnings (e.g. \"claude.ai connectors disabled …\") into
# the release body that ships to users. Stderr still streams to the
# operator's terminal so real errors stay visible.
RELEASE_NOTES=$(claude -p --system-prompt "You are a release note generator for the Manual Tool Change plugin. Write notes for end users, not developers. Only use the commit messages provided. Never invent changes. Skip internal-only changes like test fixes, refactors, and CI updates." "$PROMPT")
CLAUDE_EXIT_CODE=$?
